...
export default function ContactsImportModal({ onClose }: Props) {
  ...
  async function submit() {
    ...
    try {
      ...
      if (!res.ok || !json.ok) {
        ...
        setResult(json);
        // Refresh server-rendered contacts list in the background.
        router.refresh();
      }
    } catch (err) {
      ...
      <div className="modal-panel" style={{ maxWidth: 640 }}>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "var(--gray-600)" }}>
                      Append these contacts to your office&apos;s existing list. Nothing is deleted or modified.
                    </div>
                  </div>
                </label>
                <label
                ...
                >
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "var(--gray-600)" }}>
                      Permanently delete <strong>all existing contacts</strong> in your office (including confidential
                      ones), then import this file as the new complete list.
                    </div>
                  </div>
                </label>
              </div>
            ...
            {mode === "replace" && (
              ...
              >
                <label style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                  ...
                  />
                  <span>
                    I understand that this will permanently delete every contact currently in this office&apos;s list
        ...
        {result && (
          <div>
            ...
            >
              Import complete. Inserted <strong>{result.inserted}</strong> contact
              {result.inserted === 1 ? "" : "s"}
              {result.mode === "replace" && (
                <>
                  {" "}
                  ...
                  <tbody>
                    {result.skippedRows.map((r) => (
                      <tr key={r.row}>
                        ...
                        <td style={{ fontSize: 12 }}>{r.preview.contact_name || "—"}</td>
                        <td style={{ fontSize: 12 }}>{r.preview.account_name || "—"}</td>
